...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->model(array('user_model', 'ava_model'));
			...
			if (!is_null($user = $this->ci->user_model->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$ava_url = !is_null($ava = $this->ci->ava_model->get_ava($user->id)) ? site_url() . $ava->file_path . $ava->file_name . $ava->file_ext : site_url() . 'assets/upload/ava/no_ava.jpg';
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'full_name' => '', 'ava_url' => '', 'status' => ''));
	 ...
	 * @return	string
	 */
	function get_ava_url()
	{
		return $this->ci->session->userdata('ava_url');
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $full_name, $email, $password, $email_activation)
	{
	...
	function is_max_login_attempts_exceeded($login)
	{
		if ($this->ci->config->item('login_count_attempts', 'tank_auth')) {
			...
			return $this->ci->login_attempt_model->get_attempts_num($this->ci->input->ip_address(), $login)
	...
	private function clear_login_attempts($login)
	{
		if ($this->ci->config->item('login_count_attempts', 'tank_auth')) {
			$this->ci->load->model('login_attempt_model');
			$this->ci->login_attempt_model->clear_attempts(
